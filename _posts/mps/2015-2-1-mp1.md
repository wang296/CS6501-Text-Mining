...
Use all the review documents in the **train folder** to estimate a unigram language model $p_u(w)$ and two bigram language models (with different smoothing). When estimating the bigram language models, using linear interpolation smoothing and absolute discount smoothing based on the unigram language model $p_u(w)$ to get two different bi-gram language models, i.e., $p^L_b(w_i|w_{i-1})$ and $p^A_b(w_i|w_{i-1})$.
...
From the resulting two bigram language models, find the top 10 words that are most likely to follow the word "good", i.e., rank the words in descending order by $p^L_b(w|'good')$ and $p^A_b(w|'good')$ and output the top 10 words. Are those top 10 words the same from these two bigram language models? Explain your observation.

*HINT: to greatly reduce space complexity, you do not need to actually maintain a $V\times V$ array to maintain the counts and probabilities for bigram language models. You can use a sparse data structure, e.g., hash map, to store the seen words/bigrams, and perform the smoothing on the fly, i.e., evoke some function calls to return $p^L_b(w_i|w_{i-1})$ and $p^A_b(w_i|w_{i-1})$.* 
...
Fixing the sentence length to be 15, generate 10 sentences by sampling words from $p_u(w)$, $p^L_b(w_i|w_{i-1})$ and $p^A_b(w_i|w_{i-1})$.

**What to submit**:

1. The 10 sentences generated from $p_u(w)$, $p^L_b(w_i|w_{i-1})$ and $p^A_b(w_i|w_{i-1})$.

### 2.3 Language model evaluation (15pts)

Compute perplexity of previously estimated language models, i.e., $p_u(w)$, $p^L_b(w_i|w_{i-1})$ and $p^A_b(w_i|w_{i-1})$, on review documents in the **test folder**.
...
1. Report the mean and standard deviation of the perplexities for $\hat p_u(w)$ to smooth $\hat p^L_b(w_i|w_{i-1})$ and $\hat p^A_b(w_i|w_{i-1})$ on the test folder.     
