...
From the resulting two bigram language models, find the top 10 words that are most likely to follow the word "good", i.e., rank the words in a descending order by $p^L\_b(w|\``good")$ and $p^A\_b(w|\``good")$ and output the top 10 words. Are those top 10 words the same from these two bigram language models? Explain your observation.

*HINT: to reduce space complexity, you do not need to actually maintain a $V\times V$ array to store the counts and probabilities for the bigram language models. You can use a sparse data structure, e.g., hash map, to store the seen words/bigrams, and perform the smoothing on the fly, i.e., evoke some function calls to return $p^L\_b(w|\unicode{x201C}good")$ and $p^A\_b(w|\unicode{x201C}good")$.* 

**What to submit**:

1. The top 10 words selected from the corresponding two bigram language models.
2. Your explanation of the observations about the top words under those two bigram language models.
...
*HINT: you can use $p\_u(w)$ to generate the first word of a sentence and then sampling from the corresponding a bigram language model when generating from $p^L\_b(w\_i|w\_{i-1})$ and $p^A\_b(w\_i|w\_{i-1})$.* 
...
Compute perplexity of the previously estimated language models, i.e., $p\_u(w)$, $p^L\_b(w\_i|w\_{i-1})$ and $p^A\_b(w\_i|w\_{i-1})$, on review documents in the **test folder**.

The perplexity metric defined in our lecture slide is for one document. Follow that definition to compute perplexity for every review document in the test folder and compute the mean and standard deviation of the  resulting perplexities.
...
1. Report the mean and standard deviation of the perplexities for $\hat p\_u(w)$, $\hat p^L\_b(w\_i|w\_{i-1})$ and $\hat p^A\_b(w\_i|w\_{i-1})$ on the test folder.     
