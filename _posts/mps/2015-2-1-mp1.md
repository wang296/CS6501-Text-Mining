...
*NOTE: to smooth the unseen words in the test folder, use additive smoothing to smooth the unigram language model $p_u(w)$ by setting the parameter $\delta=0.1$. Then use the smoothed $\hat p\_u(w)$ to smooth $p^L\_b(w\_i|w\_{i-1})$ and $p^A\_b(w\_i|w\_{i-1})$.*
