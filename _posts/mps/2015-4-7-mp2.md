...
# <a name="nb"></a>Task 3: k Nearest Neighbor (20pts)

In our course lecture, we have introduced a highly efficient approximation method for retrieving the nearest neighbors to the input instance, i.e., random projection. We will implement this method to speed up kNN computation.

We will use the TF-IDF feature vector and cosine similarity between vectors as distance metric in our kNN classifier. We will follow the same definition of TF-IDF as we used in MP1, i.e., (sub-linear TF scaling). Note in Naive Bayes classifier, we only need raw TF as our feature vector.  

Denote the size of our finalized controlled vocabulary as $m$, we will create a $l$-bits hash code for every review document in the corpus via random projection. The specific details of performing random projection has been discussed in our lecture slides. Briefly, we will create $l$ different $m$-dimensional random vectors, and each dimension of these random vectors falls into the range of [-1,1]. For review document, we will take the inner product of its vector representation with those $l$ random vectors; and use the sign function to encode each bit as 0 or 1.

NOTE: those $l$ random vectors should be shared around all the review documents. In other words, you should first create those $l$ random vectors, store them, and keep using them for all the incoming documents for computing their hashing value.   

**What to submit**:
1. Copy and paste your implementation of random projection. (10pts) 
2. Use your implemented random projection to retrieve the most similar documents to those five sample review documents listed in the query json file in MP1. Report the running time for brute-force k nearest neighbor search and approximate k nearest neighbor search by random project. Can we still find reasonable results with this approximation, i.e., the retrieved documents help you determine the cuisine mentioned in the query document? 
